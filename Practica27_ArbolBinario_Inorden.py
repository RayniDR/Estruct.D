...
class ArbolBi:
	...
	def Inorden(self,nodo,string):# metodo inorden recibe la raiz del arbol
		
		if nodo:# si el nodo actual es diferente de null
			self.Inorden(nodo.left,string)# comienza recorriendo el lado izquierdo recursivamente
			print((nodo.dato))# llega a la raiz y la imprime

			self.Inorden(nodo.rigth,string)# recorre el lado derecho

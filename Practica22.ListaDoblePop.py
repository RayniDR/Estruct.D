		...
		if selec==1:#condicion para imprimir toda la lista
			...
			while actual:# mientras aun existan nodos
				...
				if actual.dato==num:# si el dato del nodo actual es igual al dato que se busca, imprime el contador
					...
					actual=actual.next
					cont+=1
	def borrar(self,elem):# funcion para borrar nodos
		actual=self.root
		conti=0
		while  actual:# Mientras sigan existiendo nodos
			if actual.dato==elem and actual==self.root: # si el dato del nodo actual es igual a lo que ingreso el usuario y es la raiz
				if actual.next:
					Ntemporal=actual.next# guarda en una variable temporal el nodo siguiente
					actual.next=None# borra el nodo siguiente
					Ntemporal.prev=None
					
					self.root=Ntemporal# hace al que era el nodo siguiente la raiz
			elif actual.dato==elem:# si el dato del nodo actual coincide con el del usuario
				
				if actual.next :# si el nodo actual tiene nodos adelante
					Ntemporal=actual.next# varible temporal que almacena los datos del nodo siguiente
					prev=actual.prev# variable que almacena los datos del nodo anterior
					prev.next=Ntemporal# pone en la posicion actual los datos del nodo siguiente
					Ntemporal.prev=prev# pone el puntero del nodo siguiente al nodo anterior
					actual.next=None# borra el nodo siguiente
					actual.prev=None$ borra el anterior
					actual=None# borra el actual
					return
				else:# si el nodo actual es el ultimo nodo
					...
					actual=None
					break
			actual=actual.next# avanza de nodo hasta que encuentre el dato
